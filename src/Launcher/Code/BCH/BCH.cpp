...
BCH<L, B, R, Q>::BCH(const int argc, const char** argv, std::ostream& stream)
  : L(argc, argv, stream)
  , params_cdc(new factory::Codec_BCH("cdc"))
{
...
template<class L, typename B, typename R, typename Q>
void
BCH<L, B, R, Q>::get_description_args()
{
...
template<class L, typename B, typename R, typename Q>
void
BCH<L, B, R, Q>::store_args()
{
    params_cdc->store(this->arg_vals);

    if (params_cdc->dec->implem == "FAST") this->params.n_frames = mipp::N<B>();
