...
template<typename R>
Channel_user<R>::Channel_user(const int N, const std::string& filename, const bool add_users)
  : Channel<R>(N)
  , add_users(add_users)
  , noise_counter(0)
{
...
template<typename R>
void
Channel_user<R>::set_noise(const size_t frame_id)
{
...
template<typename R>
void
Channel_user<R>::read_noise_file(const std::string& filename, const int N, std::vector<std::vector<R>>& noise_buffer)
{
    try
    {
...
template<typename R>
void
Channel_user<R>::read_as_text(const std::string& filename, const int N, std::vector<std::vector<R>>& noise_buffer)
{
    ...
    if (file.is_open())
    {
        ...
        if (n_fra == 0 || fra_size <= 0)
        {
            ...
            message << "'n_fra' and 'fra_size' have to be bigger than 0 ('n_fra' = " << n_fra
                    << ", 'fra_size' = " << fra_size << ").";
...
template<typename R>
void
Channel_user<R>::read_as_binary(const std::string& filename, const int N, std::vector<std::vector<R>>& noise_buffer)
{
