...
template<typename R>
Channel_Rayleigh_LLR_user<R>::Channel_Rayleigh_LLR_user(const int N,
                                                        const bool complex,
  ...
  , gain_index(0)
{
    ...
    if (complex || add_users)
        throw spu::tools::invalid_argument(
          __FILE__, __LINE__, __func__, "Arguments 'complex' and 'add_users' are not supported yet.");

    if (gain_occurrences <= 0)
        throw spu::tools::invalid_argument(
          __FILE__, __LINE__, __func__, "Argument 'gain_occurrences' must be strictly positive.");
...
template<typename R>
tools::Gaussian_gen<R>*
create_gaussian_generator(const tools::Gaussian_noise_generator_implem implem, const int seed)
{
    switch (implem)
    {
...
Channel_Rayleigh_LLR_user<R>::clone() const
{
...
template<typename R>
void
Channel_Rayleigh_LLR_user<R>::set_seed(const int seed)
{
...
template<typename R>
void
Channel_Rayleigh_LLR_user<R>::read_gains(const std::string& gains_filename)
{
    if (gains_filename.empty())
        throw spu::tools::invalid_argument(
          __FILE__, __LINE__, __func__, "Argument 'gains_filename' should not be empty.");
...
Channel_Rayleigh_LLR_user<R>::_add_noise_wg(const float* CP, const R* X_N, R* H_N, R* Y_N, const size_t /*frame_id*/)
{
    ...
    for (unsigned i = 0; i < gains.size(); ++i)
    {
        ...
        if (current_gain_occur >= gain_occur)
        {
            ...
            if (gain_index >= gains_stock.size()) gain_index = 0;
        }
    }
...
template<typename R>
void
Channel_Rayleigh_LLR_user<R>::set_n_frames(const size_t n_frames)
{
