...
Monitor_EXIT<B, R>::Monitor_EXIT(const int N, const unsigned max_n_trials)
  : Monitor()
  , N(N)
  , max_n_trials(max_n_trials)
{
    ...
    this->create_codelet(
      p,
      [ps_bits, ps_llrs_a, ps_llrs_e](spu::module::Module& m, spu::runtime::Task& t, const size_t frame_id) -> int
      {
...
template<typename B, typename R>
bool
Monitor_EXIT<B, R>::equivalent(const Monitor_EXIT<B, R>& m, bool do_throw) const
{
    if (get_N() != m.get_N())
    {
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::check_mutual_info(const B* bits,
                                      const R* llrs_a,
                                      const R* llrs_e,
                                      const int frame_id,
...
Monitor_EXIT<B, R>::_check_mutual_info(const B* bits, const R* llrs_a, const R* llrs_e, const size_t /*frame_id*/)
{
    ...
    {
        this->_check_mutual_info_avg(bits + f * get_N(), llrs_a + f * get_N(), f);
...
template<typename B, typename R>
R
Monitor_EXIT<B, R>::_check_mutual_info_histo() const
{
    ...
    else
    {
        ...
        if (llr_0_noninfinite_count && llr_1_noninfinite_count && llr_0_min <= llr_1_max && llr_1_min <= llr_0_max)
        {
            ...
            for (unsigned i = 0; i < N; i++)
            {
                ...
                {
                    if ((int)bits_buff[i] == 0)
                        llr_0_mean += llrs_e_buff[i];
                    else
                        llr_1_mean += llrs_e_buff[i];
                }
            }
            ...
            for (unsigned i = 0; i < N; i++)
            {
                ...
                {
                    if ((int)bits_buff[i] == 0)
                        llr_0_variance += std::pow((llrs_e_buff[i] - llr_0_mean), 2);
                    else
                        llr_1_variance += std::pow((llrs_e_buff[i] - llr_1_mean), 2);
                }
            }
            ...
            bin_width =
              (R)0.5 *
              ((R)3.49 * (R)std::sqrt(llr_0_variance) * (R)(std::pow(llr_0_noninfinite_count, (R)-1.0 / (R)3.0)) +
        ...
        for (unsigned i = 0; i < N; i++)
        {
            if (llrs_e_buff[i] == -inf)
                histogram[(int)bits_buff[i]][0]++;
            else if (llrs_e_buff[i] == inf)
                histogram[(int)bits_buff[i]][bin_count - 1]++;
            else
            {
                if (lots_of_bins)
                {
        ...
        for (auto b = 0; b < 2; b++)
            for (unsigned bin_ix = 0; bin_ix < bin_count; bin_ix++)
                if (pdf[b][bin_ix] > (R)0)
                    I_E +=
                      (R)0.5 * pdf[b][bin_ix] * std::log2((R)2.0 * pdf[b][bin_ix] / (pdf[0][bin_ix] + pdf[1][bin_ix]));
...
template<typename B, typename R>
bool
Monitor_EXIT<B, R>::n_trials_achieved() const
{
...
template<typename B, typename R>
bool
Monitor_EXIT<B, R>::is_done() const
{
    return n_trials_achieved();
}

template<typename B, typename R>
const typename Monitor_EXIT<B, R>::Attributes&
Monitor_EXIT<B, R>::get_attributes() const
{
    return vals;
}
...
template<typename B, typename R>
unsigned
Monitor_EXIT<B, R>::get_max_n_trials() const
{
    return max_n_trials;
}
...
template<typename B, typename R>
R
Monitor_EXIT<B, R>::get_I_A() const
{
...
template<typename B, typename R>
R
Monitor_EXIT<B, R>::get_I_E() const
{
...
template<typename B, typename R>
unsigned long long
Monitor_EXIT<B, R>::get_n_trials() const
{
...
template<typename B, typename R>
uint32_t
Monitor_EXIT<B, R>::record_callback_measure(std::function<void(void)> callback)
{
...
template<typename B, typename R>
bool
Monitor_EXIT<B, R>::unrecord_callback_measure(const uint32_t id)
{
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::clear_callbacks()
{
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::collect(const Monitor& m, bool fully)
{
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::collect(const Attributes& v)
{
    vals += v;
}
...
template<typename B, typename R>
Monitor_EXIT<B, R>&
Monitor_EXIT<B, R>::operator+=(const Monitor_EXIT<B, R>& m)
{
    collect(m, false);
    return *this;
}
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::copy(const Monitor& m, bool fully)
{
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::copy(const Attributes& v)
{
    vals = v;
}
...
template<typename B, typename R>
typename Monitor_EXIT<B, R>::Attributes&
Monitor_EXIT<B, R>::Attributes ::operator+=(const Attributes& a)
{
    n_trials += a.n_trials;
    I_A_sum += a.I_A_sum;
...
template<typename B, typename R>
void
Monitor_EXIT<B, R>::Attributes ::reset()
{
    n_trials = 0;
    I_A_sum = 0.;
}
...
template<typename B, typename R>
Monitor_EXIT<B, R>::Attributes ::Attributes()
{
    reset();
}
