...
Modem_OOK_AWGN<B, R, Q>::Modem_OOK_AWGN(const int N, const bool disable_sig2)
  : Modem_OOK<B, R, Q>(N)
...
Modem_OOK_AWGN<B, R, Q>::clone() const
{
...
Modem_OOK_AWGN<B, R, Q>::_demodulate(const float* CP, const Q* Y_N1, Q* Y_N2, const size_t /*frame_id*/)
{
    ...
    else
    {
        ...
        if (*CP != this->last_channel_param) this->sigma_factor = (R)2.0 * (*CP) * (*CP);
