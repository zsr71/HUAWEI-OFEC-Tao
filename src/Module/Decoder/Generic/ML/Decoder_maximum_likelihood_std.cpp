...
Decoder_maximum_likelihood_std<B, R>::Decoder_maximum_likelihood_std(const int K,
                                                                     const int N,
...
Decoder_maximum_likelihood_std<B, R>::clone() const
{
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_std<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_std<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    ...
    else
    {
        ...
        for (uint64_t u = 0; u <= this->u_max; u++)
        {
            ...
            if (this->u_max == std::numeric_limits<uint64_t>::max()) break;
        }
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_std<B, R>::_decode_hiho(const B* Y_N, B* V_K, const size_t frame_id)
{
...
Decoder_maximum_likelihood_std<B, R>::_decode_hiho_cw(const B* Y_N, B* V_N, const size_t /*frame_id*/)
{
    ...
    for (uint64_t u = 0; u <= this->u_max; u++)
    {
        ...
        // trick to avoid infinite loop when u_max is the max 64-bit unsigned integer
        if (u_max == std::numeric_limits<uint64_t>::max()) break;
    }
