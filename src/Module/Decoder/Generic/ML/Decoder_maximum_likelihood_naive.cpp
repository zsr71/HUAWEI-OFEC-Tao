...
Decoder_maximum_likelihood_naive<B, R>::Decoder_maximum_likelihood_naive(const int K,
                                                                         const int N,
                                                                         const Encoder<B>& encoder,
                                                                         const bool hamming)
...
Decoder_maximum_likelihood_naive<B, R>::clone() const
{
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_naive<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_naive<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    ...
    else
    {
        ...
        for (uint64_t x = 0; x <= this->x_max; x++)
        {
            ...
            if (this->x_max == std::numeric_limits<uint64_t>::max()) break;
        }
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_naive<B, R>::_decode_hiho(const B* Y_N, B* V_K, const size_t frame_id)
{
    if (!this->encoder->is_sys())
    {
        std::stringstream message;
...
template<typename B, typename R>
int
Decoder_maximum_likelihood_naive<B, R>::_decode_hiho_cw(const B* Y_N, B* V_N, const size_t /*frame_id*/)
{
    ...
    for (uint64_t x = 0; x <= this->x_max; x++)
    {
        ...
        // trick to avoid infinite loop when x_max is the max 64-bit unsigned integer
        if (x_max == std::numeric_limits<uint64_t>::max()) break;
    }
