...
Decoder_BCH_genius<B, R>::Decoder_BCH_genius(const int K, const int N, const int t, Encoder<B>& encoder)
...
template<typename B, typename R>
int
Decoder_BCH_genius<B, R>::_decode_hiho(const B* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_BCH_genius<B, R>::_decode_hiho_cw(const B* Y_N, B* V_N, const size_t frame_id)
{
    auto& X_N = encoder.get_X_N(frame_id);
...
template<typename B, typename R>
int
Decoder_BCH_genius<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_BCH_genius<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    tools::hard_decide(Y_N, this->YH_N.data(), this->N);
...
template<typename B, typename R>
void
Decoder_BCH_genius<B, R>::set_n_frames(const size_t n_frames)
{
