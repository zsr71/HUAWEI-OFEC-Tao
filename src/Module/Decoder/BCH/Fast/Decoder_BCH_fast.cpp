...
Decoder_BCH_fast<B, R>::Decoder_BCH_fast(const int& K, const int& N, const tools::BCH_polynomial_generator<B>& GF_poly)
  ...
  , index_of(GF_poly.get_index_of())
{
    ...
    this->last_is_codeword.resize(
      this->n_frames + mipp::N<B>()); // in case where only the last frame_id is called to prevent memory leak
}

template<typename B, typename R>
Decoder_BCH_fast<B, R>*
Decoder_BCH_fast<B, R>::clone() const
{
...
mipp::Reg<B>
read_array(const std::vector<B, A>& array, mipp::Reg<B> r_idx, const mipp::Msk<mipp::N<B>()>& m_idx = true)
{
...
template<typename B, class A = std::allocator<B>>
void
write_array(std::vector<mipp::Reg<B>, A>& array,
            const mipp::Reg<B>& r_idx,
            const mipp::Msk<mipp::N<B>()>& m_idx,
...
template<typename B, typename R>
int
Decoder_BCH_fast<B, R>::_decode(const size_t frame_id)
{
    const auto r_zero = mipp::Reg<B>((B)0);
    ...
    if (!mipp::testz(syn_error))
    {
        ...
        do
        {
            ...
            if (u < t2)
            {
                ...
                discrepancy[u_p1] =
                  mipp::blend(read_array(index_of, discrepancy[u_p1], m_process), discrepancy[u_p1], m_process);
            }
...
template<typename B, typename R>
int
Decoder_BCH_fast<B, R>::_decode_hiho(const B* Y_N, B* V_K, const size_t frame_id)
{
    _load(Y_N, frame_id);
    ...
    tools::Reorderer_static<B, mipp::N<B>()>::apply_rev(
      (B*)(this->Y_N_reorderered.data() + this->N - this->K), frames_rev, this->K);
...
template<typename B, typename R>
int
Decoder_BCH_fast<B, R>::_decode_hiho_cw(const B* Y_N, B* V_N, const size_t frame_id)
{
    _load(Y_N, frame_id);
...
template<typename B, typename R>
int
Decoder_BCH_fast<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_BCH_fast<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    tools::hard_decide(Y_N, this->YH_N.data(), this->N * mipp::N<B>());
...
template<typename B, typename R>
void
Decoder_BCH_fast<B, R>::set_n_frames(const size_t n_frames)
{
