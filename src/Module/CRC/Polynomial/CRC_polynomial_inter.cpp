...
template<typename B>
CRC_polynomial_inter<B>::CRC_polynomial_inter(const int K, const std::string& poly_key, const int size)
  : CRC_polynomial<B>(K, poly_key, size)
  , buff_crc_inter((this->K + this->size) * mipp::nElReg<B>())
{
...
CRC_polynomial_inter<B>::clone() const
{
...
template<typename B>
void
CRC_polynomial_inter<B>::_build(const B* U_K1, B* U_K2, const size_t frame_id)
{
    for (size_t f = 0; f < this->get_n_frames_per_wave(); f++)
        CRC_polynomial<B>::_build(U_K1 + f * this->K, U_K2 + f * (this->K + this->size), frame_id + f);
}
...
template<typename B>
void
CRC_polynomial_inter<B>::_extract(const B* V_K1, B* V_K2, const size_t frame_id)
{
    for (size_t f = 0; f < this->get_n_frames_per_wave(); f++)
        CRC_polynomial<B>::_extract(V_K1 + f * (this->K + this->size), V_K2 + f * this->K, frame_id + f);
}

template<typename B>
void
CRC_polynomial_inter<B>::_generate_INTER(const B* U_in,
                                         B* U_out,
                                         const int off_in,
                                         const int off_out,
                                         const int loop_size,
