...
Quantizer_pow2_fast<R, Q>::Quantizer_pow2_fast(const int N, const short& fixed_point_pos)
  : Quantizer<R, Q>(N)
...
template<typename R, typename Q>
Quantizer_pow2_fast<R, Q>::Quantizer_pow2_fast(const int N, const short& fixed_point_pos, const short& saturation_pos)
  : Quantizer<R, Q>(N)
...
Quantizer_pow2_fast<R, Q>::clone() const
{
...
Quantizer_pow2_fast<float, short>::_process(const float* Y_N1, short* Y_N2, const size_t /*frame_id*/)
{
    ...
    for (unsigned i = vectorized_size; i < size; i++)
        Y_N2[i] =
          (short)spu::tools::saturate((float)std::round((float)factor * Y_N1[i]), (float)val_min, (float)val_max);
}
}
}
...
Quantizer_pow2_fast<float, signed char>::_process(const float* Y_N1, signed char* Y_N2, const size_t /*frame_id*/)
{
    ...
    for (unsigned i = vectorized_size; i < size; i++)
        Y_N2[i] =
          (signed char)spu::tools::saturate((float)std::round((float)factor * Y_N1[i]), (float)val_min, (float)val_max);
}
}
}
