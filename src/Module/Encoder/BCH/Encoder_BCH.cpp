...
template<typename B>
Encoder_BCH<B>::Encoder_BCH(const int& K, const int& N, const tools::BCH_polynomial_generator<B>& GF_poly)
  : Encoder<B>(K, N)
  , n_rdncy(GF_poly.get_n_rdncy())
  , g(GF_poly.get_g())
  , bb(n_rdncy)
{
    ...
    if ((this->N - this->K) != n_rdncy)
    {
        ...
        message << "'N - K' is different than 'n_rdncy' ('K' = " << K << ", 'N' = " << N << ", 'n_rdncy' = " << n_rdncy
                << ", 'N - K' = " << (this->N - this->K) << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }

    std::iota(
      this->info_bits_pos.begin(), this->info_bits_pos.end(), n_rdncy); // redundancy on the first 'n_rdncy' bits
}
...
template<typename B>
void
Encoder_BCH<B>::__encode(const B* U_K, B* par)
{
...
template<typename B>
void
Encoder_BCH<B>::_encode(const B* U_K, B* X_N, const size_t /*frame_id*/)
{
...
template<typename B>
bool
Encoder_BCH<B>::is_codeword(const B* X_N)
{
    this->__encode(X_N + n_rdncy, this->bb.data());

    auto p = 0;
    while (p < n_rdncy && (X_N[p] == this->bb[p]))
        p++;
