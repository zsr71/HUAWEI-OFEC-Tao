...
template<typename T>
static typename Interleaver_core_row_column<T>::READ_ORDER
read_order_cvt(const std::string& read_order)
{
    if (read_order == "TOP_LEFT") return Interleaver_core_row_column<T>::READ_ORDER::TOP_LEFT;
    if (read_order == "TOP_RIGHT") return Interleaver_core_row_column<T>::READ_ORDER::TOP_RIGHT;
    if (read_order == "BOTTOM_LEFT") return Interleaver_core_row_column<T>::READ_ORDER::BOTTOM_LEFT;
    if (read_order == "BOTTOM_RIGHT") return Interleaver_core_row_column<T>::READ_ORDER::BOTTOM_RIGHT;
...
template<typename T>
Interleaver_core_row_column<T>::Interleaver_core_row_column(const int size,
                                                            const int n_cols,
...
Interleaver_core_row_column<T>::clone() const
{
...
template<typename T>
void
Interleaver_core_row_column<T>::gen_lut(T* lut, const size_t /*frame_id*/)
{
    T idx = 0;
    switch (read_order)
    {
