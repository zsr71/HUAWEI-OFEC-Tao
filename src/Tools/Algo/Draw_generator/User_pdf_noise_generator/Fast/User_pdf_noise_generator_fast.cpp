...
template<typename R>
User_pdf_noise_generator_fast<R>::User_pdf_noise_generator_fast(const tools::Distributions<R>& dists,
                                                                const int seed,
...
User_pdf_noise_generator_fast<R>::clone() const
{
...
template<typename R>
void
User_pdf_noise_generator_fast<R>::set_seed(const int seed)
{
    mt19937.seed(seed);
...
template<typename R>
mipp::Reg<R>
User_pdf_noise_generator_fast<R>::get_random_simd()
{
...
template<typename R>
R
User_pdf_noise_generator_fast<R>::get_random()
{
    throw spu::tools::runtime_error(
      __FILE__, __LINE__, __func__, "The MT19937 random generator does not support this type.");
}
...
template<>
mipp::Reg<float>
User_pdf_noise_generator_fast<float>::get_random_simd()
{
...
template<>
float
User_pdf_noise_generator_fast<float>::get_random()
{
...
template<>
void
User_pdf_noise_generator_fast<float>::generate(const float* signal,
                                               float* draw,
                                               const unsigned length,
