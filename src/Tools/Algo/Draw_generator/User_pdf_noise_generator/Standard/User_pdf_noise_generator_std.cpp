...
template<typename R>
User_pdf_noise_generator_std<R>::User_pdf_noise_generator_std(const tools::Distributions<R>& dists,
                                                              const int seed,
...
User_pdf_noise_generator_std<R>::clone() const
{
...
template<typename R>
void
User_pdf_noise_generator_std<R>::set_seed(const int seed)
{
    rd_engine.seed(seed);
}
...
template<typename R>
void
User_pdf_noise_generator_std<R>::generate(const R* signal, R* draw, const unsigned length, const R noise_power)
{
    ...
    for (unsigned i = 0; i < length; i++)
    {
        ...
        draw[i] = interp_function(
          cdf_y.data(), cdf_x.data(), (const unsigned)cdf_x.size(), this->uniform_dist(this->rd_engine));
    }
}
