...
void
Matrix ::self_turn(Way w)
{
    if (w == Way::VERTICAL)
    {
        if (get_n_cols() > get_n_rows()) this->self_transpose();
    }
...
Matrix::Way
Matrix ::get_way() const
{
    return (get_n_cols() >= get_n_rows()) ? Way::HORIZONTAL : Way::VERTICAL;
}

float
Matrix ::compute_density() const
{
    return ((float)n_connections / (float)(n_rows * n_cols));
}

void
Matrix ::check_indexes(const size_t row_index, const size_t col_index) const
{
...
void
Matrix ::self_resize(const size_t _n_rows, const size_t _n_cols)
{
    n_rows = _n_rows;
    n_cols = _n_cols;
}
...
bool
Matrix ::is_of_way(Way w) const noexcept
{
    return get_way() == w;
}

void
Matrix ::is_of_way_throw(Way w) const
{
    if (!is_of_way(w))
    {
        std::stringstream message;
...
Matrix ::way_to_str(Way w)
{
    ...
    if (str.empty()) // this 'if' is a test outside the switch case (instead of default) to keep the compiler check that
                     // all cases of 'Way' are well represented.
    {
        std::stringstream message;
...
std::ostream&
operator<<(std::ostream& os, const Matrix& sm)
{
    sm.print(0, os);
    return os;
}