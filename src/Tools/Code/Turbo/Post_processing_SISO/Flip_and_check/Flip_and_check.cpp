...
Flip_and_check<B, R>::Flip_and_check(const int K,
                                     const int n_ite,
...
template<typename B, typename R>
Flip_and_check<B, R>*
Flip_and_check<B, R>::clone() const
{
...
template<typename B, typename R>
bool
Flip_and_check<B, R>::siso_n(const int ite, const mipp::vector<R>& sys, mipp::vector<R>& ext, mipp::vector<B>& s)
{
...
template<typename R>
mipp::vector<unsigned int>
partial_sort_indexes(const mipp::vector<R>& v)
{
    ...
    std::partial_sort(idx.begin(), idx.begin() + 20, idx.end(), [&v](size_t i1, size_t i2) { return v[i1] < v[i2]; });
    return idx;
}

template<typename B, typename R>
bool
Flip_and_check<B, R>::apply_flip_and_check(const mipp::vector<R>& sys, const mipp::vector<R>& ext, mipp::vector<B>& s)
{
    ...
    if (check_crc) std::copy(s_tmp.begin(), s_tmp.end(), s.begin());
...
template<typename B, typename R>
void
Flip_and_check<B, R>::set_n_frames(const size_t n_frames)
{
