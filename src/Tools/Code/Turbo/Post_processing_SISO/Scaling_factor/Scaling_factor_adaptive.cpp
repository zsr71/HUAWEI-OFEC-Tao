...
Scaling_factor_adaptive<B, R>::Scaling_factor_adaptive(const int n_ite)
                                      ...
                                      mipp::vector<B>& /*s*/)
{
    ...
    /*	else if (ite == this->n_ite) // sf = 1.00
        ...
        for (auto i = 0; i < loop_size; i++)
            ext[i] *= (R)0.85; // spu::tools::div4<R>(spu::tools::div2<R>(ext[i])) +
                               // spu::tools::div4<R>(spu::tools::div2<R>(ext[i])) +
                               // spu::tools::div4<R>(spu::tools::div2<R>(ext[i])) + spu::tools::div2<R>(ext[i]);
...
Scaling_factor_adaptive<B, R>::clone() const
{
