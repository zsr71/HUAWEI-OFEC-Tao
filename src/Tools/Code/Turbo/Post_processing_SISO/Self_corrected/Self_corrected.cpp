...
Self_corrected<B, R>::Self_corrected(const int K, const int n_ite, const int m, const int M)
...
template<typename B, typename R>
Self_corrected<B, R>*
Self_corrected<B, R>::clone() const
{
                             ...
                             mipp::vector<B>& /*s*/)
{
    if (ite <= M)
    {
        ...
        if (ite > 1)
        {
            ...
            if (ite >= m) this->correct(this->osc_nat, this->previously_corrected_nat, ext);
        }
    }
...
Self_corrected<B, R>::siso_i(const int ite, const mipp::vector<R>& /*sys*/, mipp::vector<R>& ext)
{
    if (ite <= M)
    {
        ...
        if (ite > 1)
        {
            ...
            if (ite >= m) this->correct(this->osc_int, this->previously_corrected_int, ext);
        }
    }
...
template<typename B, typename R>
inline void
Self_corrected<B, R>::collect(const int ite, const mipp::vector<R>& cur_ext, std::vector<mipp::vector<R>>& ext_hist)
{
...
template<typename B, typename R>
inline void
Self_corrected<B, R>::calc_osc(const int ite, const std::vector<mipp::vector<R>>& ext_hist, mipp::vector<B>& osc)
{
    B sgn_cur, sgn_prev;
...
template<typename B, typename R>
inline void
Self_corrected<B, R>::correct(const mipp::vector<B>& osc, mipp::vector<B>& prev_corr, mipp::vector<R>& ext)
{
...
template<typename B, typename R>
void
Self_corrected<B, R>::set_n_frames(const size_t n_frames)
{
