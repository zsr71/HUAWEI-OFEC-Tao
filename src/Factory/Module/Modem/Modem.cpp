...
Modem ::Modem(const std::string& prefix)
...
void
Modem ::get_description(cli::Argument_map_info& args) const
{
    ...
    tools::add_arg(args, p, class_name + "p+bps", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+const-path", cli::File(cli::openmode::read_write));

    tools::add_arg(args, p, class_name + "p+cb-path", cli::File(cli::openmode::read));

    tools::add_arg(args, p, class_name + "p+cpm-std", cli::Text(cli::Including_set("GSM")));

    tools::add_arg(args, p, class_name + "p+cpm-L", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+cpm-k", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+cpm-p", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+cpm-upf", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+cpm-map", cli::Text(cli::Including_set("NATURAL", "GRAY")));

    tools::add_arg(args, p, class_name + "p+cpm-ws", cli::Text(cli::Including_set("GMSK", "REC", "RCOS")));

    // --------------------------------------------------------------------------------------------------- demodulator
    tools::add_arg(args, p, class_name + "p+max", cli::Text(cli::Including_set("MAX", "MAXL", "MAXS", "MAXSS")));

    tools::add_arg(args, p, class_name + "p+no-sig2", cli::None());

    tools::add_arg(args, p, class_name + "p+psi", cli::Text(cli::Including_set("PSI0", "PSI1", "PSI2", "PSI3")));

    tools::add_arg(args, p, class_name + "p+ite", cli::Integer(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+rop-est", cli::Integer(cli::Positive()));
}

void
Modem ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
    ...
    if (this->type == "BPSK" || this->type == "OOK") this->bps = 1;
    ...
    if (cstl != nullptr && this->type == "USER") this->bps = cstl->get_n_bits_per_symbol();

    this->complex = is_complex_mod(this->type, this->bps, cstl.get());

    this->N_mod = get_buffer_size_after_modulation(
      this->type, this->N, this->bps, this->cpm_upf, this->cpm_L, this->cpm_p, cstl.get());

    this->N_fil = get_buffer_size_after_filtering(this->type, this->N, this->bps, this->cpm_L, this->cpm_p, cstl.get());
...
void
Modem ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    auto p = this->get_prefix();
    ...
    if (this->type == "CPM")
    {
        if (this->cpm_std.size()) headers[p].push_back(std::make_pair("CPM standard", this->cpm_std));

        headers[p].push_back(std::make_pair("CPM L memory", std::to_string(this->cpm_L)));
        headers[p].push_back(std::make_pair(
          "CPM h index", (std::to_string(this->cpm_k) + std::string("/") + std::to_string(this->cpm_p))));
    ...
    std::string demod_max =
      (this->type == "BPSK") || (this->type == "OOK") || (this->type == "SCMA") ? "unused" : this->max;
    ...
    if (demod_max != "unused") headers[p].push_back(std::make_pair("Max type", demod_max));
    if (this->type == "SCMA")
    {
    ...
    if (this->type == "OOK" && channel_type == "OPTICAL")
    {
        ...
        if (this->rop_est_bits > 0) str_est = "on " + std::to_string(this->rop_est_bits) + " bits";
        headers[p].push_back(std::make_pair("ROP estimation", str_est));
    }
}

template<typename R>
tools::Constellation<R>*
Modem ::build_constellation() const
{
...
module::Modem<B, R, Q>*
Modem ::_build(const tools::Constellation<R>* cstl) const
{
    if (this->type == "BPSK" && this->implem == "STD") return new module::Modem_BPSK<B, R, Q>(this->N, this->no_sig2);
    if (this->type == "BPSK" && this->implem == "FAST")
        return new module::Modem_BPSK_fast<B, R, Q>(this->N, this->no_sig2);
    if (this->type == "CPM" && this->implem == "STD")
        return new module::Modem_CPM<B, R, Q, MAX>(this->N,
                                                   this->bps,
                                                   this->cpm_upf,
                                                   this->cpm_L,
                                                   this->cpm_k,
...
module::Modem<B, R, Q>*
Modem ::_build_scma() const
{
    if (this->psi == "PSI0")
        return new module::Modem_SCMA<B, R, Q, tools::psi_0<Q>>(
          this->N, this->codebook_path, this->no_sig2, this->n_ite);
    if (this->psi == "PSI1")
        return new module::Modem_SCMA<B, R, Q, tools::psi_1<Q>>(
          this->N, this->codebook_path, this->no_sig2, this->n_ite);
    if (this->psi == "PSI2")
        return new module::Modem_SCMA<B, R, Q, tools::psi_2<Q>>(
          this->N, this->codebook_path, this->no_sig2, this->n_ite);
    if (this->psi == "PSI3")
        return new module::Modem_SCMA<B, R, Q, tools::psi_3<Q>>(
          this->N, this->codebook_path, this->no_sig2, this->n_ite);
...
template<typename B, typename R, typename Q>
module::Modem<B, R, Q>*
Modem ::build(const tools::Constellation<R>* cstl) const
{
...
template<typename B, typename R, typename Q>
module::Modem<B, R, Q>*
Modem ::build(const tools::Distributions<R>& dist) const
{
...
bool
Modem ::has_constellation(const std::string& type)
{
    return type == "PAM" || type == "QAM" || type == "PSK" || type == "USER";
}

int
Modem ::get_buffer_size_after_modulation(const std::string& type,
                                         const int N,
                                         const int bps,
                                         const int cpm_upf,
                                         const int cpm_L,
...
{
    if (c != nullptr && has_constellation(type)) return module::Modem_generic<>::size_mod(N, *c);
...
int
Modem ::get_buffer_size_after_filtering(const std::string& type,
                                        const int N,
                                        const int bps,
                                        const int cpm_L,
                                        const int cpm_p,
                                        const tools::Constellation<float>* c)
{
    if (c != nullptr && has_constellation(type)) return module::Modem_generic<>::size_fil(N, *c);
...
template aff3ct::module::Modem<B_8, R_8, Q_8>*
aff3ct::factory::Modem::build<B_8, R_8, Q_8>(const tools::Constellation<R_8>*) const;
template aff3ct::module::Modem<B_8, R_8, R_8>*
aff3ct::factory::Modem::build<B_8, R_8, R_8>(const tools::Constellation<R_8>*) const;
template aff3ct::module::Modem<B_16, R_16, Q_16>*
aff3ct::factory::Modem::build<B_16, R_16, Q_16>(const tools::Constellation<R_16>*) const;
template aff3ct::module::Modem<B_16, R_16, R_16>*
aff3ct::factory::Modem::build<B_16, R_16, R_16>(const tools::Constellation<R_16>*) const;
template aff3ct::module::Modem<B_32, R_32, Q_32>*
aff3ct::factory::Modem::build<B_32, R_32, Q_32>(const tools::Constellation<R_32>*) const;
template aff3ct::module::Modem<B_64, R_64, Q_64>*
aff3ct::factory::Modem::build<B_64, R_64, Q_64>(const tools::Constellation<R_64>*) const;

template aff3ct::module::Modem<B_8, R_8, Q_8>*
aff3ct::factory::Modem::build<B_8, R_8, Q_8>(const tools::Distributions<R_8>&) const;
template aff3ct::module::Modem<B_8, R_8, R_8>*
aff3ct::factory::Modem::build<B_8, R_8, R_8>(const tools::Distributions<R_8>&) const;
template aff3ct::module::Modem<B_16, R_16, Q_16>*
aff3ct::factory::Modem::build<B_16, R_16, Q_16>(const tools::Distributions<R_16>&) const;
template aff3ct::module::Modem<B_16, R_16, R_16>*
aff3ct::factory::Modem::build<B_16, R_16, R_16>(const tools::Distributions<R_16>&) const;
template aff3ct::module::Modem<B_32, R_32, Q_32>*
aff3ct::factory::Modem::build<B_32, R_32, Q_32>(const tools::Distributions<R_32>&) const;
template aff3ct::module::Modem<B_64, R_64, Q_64>*
aff3ct::factory::Modem::build<B_64, R_64, Q_64>(const tools::Distributions<R_64>&) const;

template tools::Constellation<R_32>*
aff3ct::factory::Modem::build_constellation<R_32>() const;
template tools::Constellation<R_64>*
aff3ct::factory::Modem::build_constellation<R_64>() const;
#else
template aff3ct::module::Modem<B, R, Q>*
aff3ct::factory::Modem::build<B, R, Q>(const tools::Constellation<R>*) const;

template aff3ct::module::Modem<B, R, Q>*
aff3ct::factory::Modem::build<B, R, Q>(const tools::Distributions<R>&) const;
#if !defined(AFF3CT_32BIT_PREC) && !defined(AFF3CT_64BIT_PREC)
template aff3ct::module::Modem<B, R, R>*
aff3ct::factory::Modem::build<B, R, R>(const tools::Constellation<R>*) const;

template aff3ct::module::Modem<B, R, R>*
aff3ct::factory::Modem::build<B, R, R>(const tools::Distributions<R>&) const;
#endif
template tools::Constellation<R>*
aff3ct::factory::Modem::build_constellation<R>() const;
#endif
