...
Scaling_factor ::Scaling_factor(const std::string& prefix)
...
void
Scaling_factor ::get_description(cli::Argument_map_info& args) const
{
    auto p = this->get_prefix();
    const std::string class_name = "factory::Scaling_factor::";

    tools::add_arg(
      args, p, class_name + "p+type", cli::Text(cli::Including_set("CST", "LTE", "LTE_VEC", "ARRAY", "ADAPTIVE")));

    tools::add_arg(args, p, class_name + "p+ite", cli::Integer(cli::Positive(), cli::Non_zero()));
}

void
Scaling_factor ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
...
Scaling_factor ::get_headers(std::map<std::string, tools::header_list>& headers, const bool /*full*/) const
{
    ...
    if (this->enable)
    {
        ...
        if (this->type == "CST") headers[p].push_back(std::make_pair("SF constant", std::to_string(this->cst)));
    }
    else
    {
...
tools::Scaling_factor<B, Q>*
Scaling_factor ::build() const
{
...
template aff3ct::tools::Scaling_factor<B_8, Q_8>*
aff3ct::factory::Scaling_factor::build<B_8, Q_8>() const;
template aff3ct::tools::Scaling_factor<B_16, Q_16>*
aff3ct::factory::Scaling_factor::build<B_16, Q_16>() const;
template aff3ct::tools::Scaling_factor<B_32, Q_32>*
aff3ct::factory::Scaling_factor::build<B_32, Q_32>() const;
template aff3ct::tools::Scaling_factor<B_64, Q_64>*
aff3ct::factory::Scaling_factor::build<B_64, Q_64>() const;
#else
template aff3ct::tools::Scaling_factor<B, Q>*
aff3ct::factory::Scaling_factor::build<B, Q>() const;
#endif
// ==================================================================================== explicit template instantiation
