...
Noise ::Noise(const std::string& prefix)
...
void
Noise ::get_description(cli::Argument_map_info& args) const
{
    ...
    tools::add_arg(
      args,
      p,
      class_name + "p+noise-range,R",
      cli::Matlab_vector<float>(cli::Real(), std::make_tuple(cli::Length(1)), std::make_tuple(cli::Length(1, 3))),
      cli::arg_rank::REQ);

    tools::add_arg(args, p, class_name + "p+noise-min,m", cli::Real(), cli::arg_rank::REQ);

    tools::add_arg(args, p, class_name + "p+noise-max,M", cli::Real(), cli::arg_rank::REQ);

    tools::add_arg(args, p, class_name + "p+noise-step,s", cli::Real(cli::Positive(), cli::Non_zero()));

    tools::add_arg(args, p, class_name + "p+pdf-path", cli::File(cli::openmode::read));
    ...
    tools::add_arg(args, p, class_name + "p+noise-type,E", cli::Text(cli::Including_set("ESN0", "EBN0", "ROP", "EP")));
}

void
Noise ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
    ...
    if (vals.exist({ p + "-pdf-path" }))
    {
        ...
        else
        {
            ...
            {
                auto it =
                  std::lower_bound(this->range.begin(), this->range.end(), vals.to_float({ p + "-noise-min", "m" }));
            ...
            {
                auto it =
                  std::upper_bound(this->range.begin(), this->range.end(), vals.to_float({ p + "-noise-max", "M" }));
...
Noise ::get_headers(std::map<std::string, tools::header_list>& headers, const bool /*full*/) const
{
    ...
    if (!this->pdf_path.empty()) headers[p].push_back(std::make_pair("PDF path", this->pdf_path));
}

template<typename R>
void
Noise ::update(tools::Noise<R>& noise, R noise_val, R bit_rate, int bps, int upf) const
{
...
template<typename R>
tools::Noise<R>*
Noise ::build(R noise_val, R bit_rate, int bps, int upf) const
{
...
template aff3ct::tools::Noise<float>*
aff3ct::factory::Noise::build<float>(float, float, int, int) const;
template aff3ct::tools::Noise<double>*
aff3ct::factory::Noise::build<double>(double, double, int, int) const;
template aff3ct::tools::Noise<float>*
aff3ct::factory::Noise::build<float>() const;
template aff3ct::tools::Noise<double>*
aff3ct::factory::Noise::build<double>() const;
template void
aff3ct::factory::Noise::update<float>(tools::Noise<float>&, float, float, int, int) const;
template void
aff3ct::factory::Noise::update<double>(tools::Noise<double>&, double, double, int, int) const;
// ==================================================================================== explicit template instantiation
