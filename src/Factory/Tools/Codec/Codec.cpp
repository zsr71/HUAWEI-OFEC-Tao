...
Codec ::Codec(const std::string& prefix)
  : Factory(Codec_name, Codec_name, prefix)
{
}

Codec ::Codec(const std::string& name, const std::string& prefix)
...
std::vector<std::string>
Codec ::get_short_names() const
{
    auto sn = Factory::get_short_names();
    if (enc != nullptr)
    {
        auto nn = enc->get_short_names();
        for (auto& x : nn)
            sn.push_back(x);
    }
    if (dec != nullptr)
    {
        auto nn = dec->get_short_names();
        for (auto& x : nn)
            sn.push_back(x);
    }
    if (pct != nullptr)
    {
        auto nn = pct->get_short_names();
        for (auto& x : nn)
            sn.push_back(x);
    }
    if (itl != nullptr)
...
std::vector<std::string>
Codec ::get_prefixes() const
{
    auto p = Factory::get_prefixes();
    if (enc != nullptr)
    {
        auto nn = enc->get_prefixes();
        for (auto& x : nn)
            p.push_back(x);
    }
    if (dec != nullptr)
    {
        auto nn = dec->get_prefixes();
        for (auto& x : nn)
            p.push_back(x);
    }
    if (pct != nullptr)
    {
        auto nn = pct->get_prefixes();
        for (auto& x : nn)
            p.push_back(x);
    }
    if (itl != nullptr)
...
void
Codec ::set_enc(Encoder* enc)
{
    this->enc.reset(enc);
}

void
Codec ::set_dec(Decoder* dec)
{
    this->dec.reset(dec);
}

void
Codec ::set_pct(Puncturer* pct)
{
    this->pct.reset(pct);
}

void
Codec ::set_itl(Interleaver* itl)
{
    this->itl.reset(itl);
}

void
Codec ::set_enc(tools::auto_cloned_unique_ptr<Encoder>&& enc)
{
    this->enc = std::move(enc);
}

void
Codec ::set_dec(tools::auto_cloned_unique_ptr<Decoder>&& dec)
{
    this->dec = std::move(dec);
}

void
Codec ::set_pct(tools::auto_cloned_unique_ptr<Puncturer>&& pct)
{
    this->pct = std::move(pct);
}

void
Codec ::set_itl(tools::auto_cloned_unique_ptr<Interleaver>&& itl)
{
