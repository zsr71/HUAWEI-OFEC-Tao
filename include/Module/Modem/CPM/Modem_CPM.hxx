...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
Modem_CPM<B, R, Q, MAX>::Modem_CPM(const int N,
  ...
  , no_sig2(no_sig2)
  , cpm(
      new tools::CPM_parameters<SIN, SOUT>(cpm_L, cpm_k, cpm_p, bits_per_symbol, sampling_factor, "TOTAL", wave_shape))
  ...
  , bcjr(*cpm, n_sy_tl)
{
    ...
    cpe.generate_mapper(cpm->transition_to_binary, cpm->binary_to_transition, mapping);

    cpe.generate_trellis(cpm->trellis_next_state, cpm->trellis_related_wave_form);
    cpe.generate_anti_trellis(cpm->anti_trellis_original_state, cpm->anti_trellis_input_transition);

    cpe.generate_tail_symb_transition();

    generate_baseband();

    if (no_sig2) this->generate_projection((R)1);
}

template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
Modem_CPM<B, R, Q, MAX>*
Modem_CPM<B, R, Q, MAX>::clone() const
{
...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
bool
Modem_CPM<B, R, Q, MAX>::is_complex_mod()
{
    return true;
}
...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
bool
Modem_CPM<B, R, Q, MAX>::is_complex_fil()
{
    return false;
}
...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
int
Modem_CPM<B, R, Q, MAX>::size_mod(const int N, const int bps, const int L, const int p, const int ups)
{
...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
int
Modem_CPM<B, R, Q, MAX>::size_fil(const int N, const int bps, const int L, const int p)
{
    int m_order = (int)1 << bps;
    int n_tl = (int)(std::ceil((float)(p - 1) / (float)(m_order - 1))) + L - 1;
...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
void
Modem_CPM<B, R, Q, MAX>::generate_baseband()
{
    ...
    for (auto wa = 0; wa < cpm->n_wa; wa++)
    {
        ...
        {
            R tilted_phase =
              tilted_phase_part1 + (R)M_PI * cpm_h *
                                     (4 * tilted_phase_part2[s] +
...
class GMSK
{
  ...
  public:
    ...
    R operator()(const R& t) const { return apply(t); }
};

template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
R
Modem_CPM<B, R, Q, MAX>::calculate_phase_response(const R t_stamp)
{
    if (cpm->wave_shape == "GMSK")
    {
        if (t_stamp == (R)0.0) return (R)0.0;
...
template<typename B, typename R, typename Q, tools::proto_max<Q> MAX>
void
Modem_CPM<B, R, Q, MAX>::generate_projection(const R factor)
{
