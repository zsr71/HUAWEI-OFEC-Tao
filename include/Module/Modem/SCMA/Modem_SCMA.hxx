...
Modem_SCMA<B, R, Q, PSI>::Modem_SCMA(const int N,
...
Modem_SCMA<B, R, Q, PSI>::clone() const
{
                                         ...
                                         const size_t /*frame_id*/)
{
    ...
    if (*CP != this->last_channel_param && !this->disable_sig2) this->n0 = ((R)4.0 * *CP * *CP);
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
void
Modem_SCMA<B, R, Q, PSI>::demodulate_batch(Q* Y_N2, int batch)
{
    ...
    for (auto itrr = 0; itrr < n_ite; itrr++)
    {
        ...
        for (auto i = 0; i < CB.get_codebook_size(); i++)
            for (auto j = 0; j < CB.get_codebook_size(); j++)
                for (auto k = 0; k < CB.get_codebook_size(); k++)
                    for (auto re = 0; re < CB.get_number_of_resources(); re++)
                    {
                        msg_resource_to_users(re, CB.get_resource_to_user(re, 0), i) +=
                          arr_phi(re, i, j, k) * msg_user_to_resources(CB.get_resource_to_user(re, 1), re, j) *
                          msg_user_to_resources(CB.get_resource_to_user(re, 2), re, k);
                        msg_resource_to_users(re, CB.get_resource_to_user(re, 1), i) +=
                          arr_phi(re, j, i, k) * msg_user_to_resources(CB.get_resource_to_user(re, 0), re, j) *
                          msg_user_to_resources(CB.get_resource_to_user(re, 2), re, k);
                        msg_resource_to_users(re, CB.get_resource_to_user(re, 2), i) +=
                          arr_phi(re, j, k, i) * msg_user_to_resources(CB.get_resource_to_user(re, 0), re, j) *
        ...
        // 						if (u2 != u)
        // 							proba *=
        // msg_user_to_resources(CB.get_resource_to_user(re,u2),re,j); ///// <------- but I'm having issue with this j
        // that shall be k once on two.
        ...
        for (auto i = 0; i < CB.get_codebook_size(); i++) // codeword index
        {
            ...
            {
                msg_user_to_resources(j, CB.get_user_to_resource(j, 0), i) =
                  normalize_prob_msg_resource_to_users(j, 1, i);
                msg_user_to_resources(j, CB.get_user_to_resource(j, 1), i) =
                  normalize_prob_msg_resource_to_users(j, 0, i);
            }
        }
    }
    // end of iterations
    ...
    for (auto i = 0; i < CB.get_codebook_size(); i++) // codeword index
    {
        for (auto j = 0; j < CB.get_number_of_users(); j++) // user index
        {
            ...
            guess(j, i) = msg_resource_to_users(CB.get_user_to_resource(j, 0), j, i) *
                          msg_resource_to_users(CB.get_user_to_resource(j, 1), j, i);
        }
    }
    ...
    {
        Y_N2[i * this->N + batch * 2 + 0] =
          (Q)(std::log(guess(i, 0) + guess(i, 2)) - std::log(guess(i, 1) + guess(i, 3)));
    }
    ...
    if ((this->N % 2) != 1 || batch != ((this->N / 2 + 1) - 1))
    {
        for (auto i = 0; i < CB.get_number_of_users(); i++) // user index
        {
            Y_N2[i * this->N + batch * 2 + 1] =
              (Q)(std::log(guess(i, 0) + guess(i, 1)) - std::log(guess(i, 2) + guess(i, 3)));
        }
    }
}
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
Q
Modem_SCMA<B, R, Q, PSI>::phi(const Q* Y_N1, int i, int j, int k, int re, int batch)
{
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
Q
Modem_SCMA<B, R, Q, PSI>::normalize_prob_msg_resource_to_users(int user, int resource_ind, int codeword)
{
    Q sum = 0;
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
Q
Modem_SCMA<B, R, Q, PSI>::phi(const Q* Y_N1, int i, int j, int k, int re, int batch, const R* H_N)
{
    Q phi;
    std::complex<Q> tmp;
    ...
    const auto H_N0 = std::complex<Q>(
      (Q)H_N[CB.get_resource_to_user(re, 0) * Nmod + CB.get_number_of_real_symbols() * batch + 2 * re],
      (Q)H_N[CB.get_resource_to_user(re, 0) * Nmod + CB.get_number_of_real_symbols() * batch + 2 * re + 1]);
    const auto H_N1 = std::complex<Q>(
      (Q)H_N[CB.get_resource_to_user(re, 1) * Nmod + CB.get_number_of_real_symbols() * batch + 2 * re],
      (Q)H_N[CB.get_resource_to_user(re, 1) * Nmod + CB.get_number_of_real_symbols() * batch + 2 * re + 1]);
    const auto H_N2 = std::complex<Q>(
      (Q)H_N[CB.get_resource_to_user(re, 2) * Nmod + CB.get_number_of_real_symbols() * batch + 2 * re],
      (Q)H_N[CB.get_resource_to_user(re, 2) * Nmod + CB.get_number_of_real_symbols() * batch + 2 * re + 1]);
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
bool
Modem_SCMA<B, R, Q, PSI>::is_complex_mod()
{
    return true;
}
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
bool
Modem_SCMA<B, R, Q, PSI>::is_complex_fil()
{
    return true;
}
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
int
Modem_SCMA<B, R, Q, PSI>::size_mod(const int N, const int bps)
{
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
int
Modem_SCMA<B, R, Q, PSI>::size_fil(const int N, const int bps)
{
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
int
Modem_SCMA<B, R, Q, PSI>::size_mod(const int N, const std::string& codebook_path)
{
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
int
Modem_SCMA<B, R, Q, PSI>::size_fil(const int N, const std::string& codebook_path)
{
    const tools::Codebook<R> CB(codebook_path);
    int bps = (int)CB.get_system_bps();
    return size_mod(N, bps);
...
template<typename B, typename R, typename Q, tools::proto_psi<Q> PSI>
void
Modem_SCMA<B, R, Q, PSI>::set_n_frames(const size_t n_frames)
{
    ...
    if (old_n_frames != n_frames) Modem<B, R, Q>::set_n_frames(n_frames);
}
