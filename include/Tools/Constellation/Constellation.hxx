...
template<typename R>
bool
has_complex_symbols(const Constellation<R>& cstl)
{
    for (unsigned i = 0; i < cstl.get_n_symbols(); i++)
    {
        if (cstl[i].imag() != (R)0.) return true;
    }
...
template<typename R>
Constellation<R>::Constellation(std::vector<S>&& symbols, const std::string& name)
  ...
  , is_cplx(true)
{
    if (constellation.size() != this->get_n_symbols())
    {
        ...
        message << "'symbols.size()' has to be a power of 2 ('symbols.size()' = " << symbols.size() << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
}

template<typename R>
Constellation<R>::Constellation(const unsigned n_bps, const std::string& name)
  : Constellation(std::move(std::vector<S>((size_t)((int64_t)1 << n_bps))), name)
{
}

template<typename R>
Constellation<R>::Constellation(const std::vector<S>& symbols, const std::string& name)
...
template<typename R>
const std::string&
Constellation<R>::get_name() const
{
    return name;
}
...
template<typename R>
unsigned
Constellation<R>::get_n_bits_per_symbol() const
{
    return n_bps;
}
...
template<typename R>
unsigned
Constellation<R>::get_n_symbols() const
{
    return n_symbs;
}
...
template<typename R>
bool
Constellation<R>::is_complex() const
{
    return is_cplx;
}

template<typename R>
const typename Constellation<R>::S&
Constellation<R>::operator[](const size_t idx) const
{
...
template<typename R>
const typename Constellation<R>::S&
Constellation<R>::get_symbol(const size_t idx) const
{
    return constellation[idx];
}
...
template<typename R>
R
Constellation<R>::get_imag(const size_t idx) const
{
...
template<typename R>
R
Constellation<R>::get_real(const size_t idx) const
{
